...
def add_ground_truth(df, debug=False):
	...
	load_dict()
	df = df.reset_index()
	df['Ground Truth'] = df.apply(lambda row: ground_truth(row['Relationship'], row['Subject']), axis=1)
...
def Analyse(message, alt_url='https://api.rosette.com/rest/v1/'):
    ...
    try:
        ...
        try:
            ...
            first_entity_id = get_id(first_entity)
            first_entity = first_entity.split(" ")
        except:
            first_entity_id = -1
        #print(first_entity_id)
        for r in RESULT['relationships']:
            ...
            if first_entity:
                if any(s in arg2_split for s in first_entity):
                    if get_id(r['arg2']) == first_entity_id:
